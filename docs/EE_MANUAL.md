...
- GI 在每层重现：不是普适常数，是经济洼地。BGM 架构 GI=4 最优；EE Self (n=3) GI=5 信号最优，GI=4 是保守运行点
...
|-------|--------|---------|------|
| 2 | 1 | ~2 | 双腔体无需长周期 |
| **3** | **5** | **5** | GI=4 保守运行点 (0.0890 vs GI=5 的 0.0950) |
| 5 | 1 | ~3 | 多腔体需更频繁同步 |
| 8 | 3 | ~5 | GI×(n-1) 不是常数 |

**关键发现：** GI=4 是 BGM 架构的最优，不是 EE Self 架构的信号最优。n=3 标准 Self 下 GI=5 最优，GI=4 保留约 94% 信号并提供一格安全边际。差异来自两个架构的信息经济学不同——BGM 的 G0 是外部观察者，EE 的 Self 是腔体间双向通信。不同的博弈结构产生不同的最优通信节律。

GI 的约束来自兰道尔账单。每 GI 步系统至少进行一次 induction_clean（不可逆剪枝）。太小 → 账单过密，帧来不及积累结构就被清理，腔体差异被同步压平。太大 → 账单过稀，帧经济拥挤，stress 驱动 τ 进入 LOCKED。GI=5 在 n=3 Self 上是信号最优点，也是接近上限的通信经济边界；GI=4 是实际实验中更保守的运行点。

注意：CALIB-GI 使用几何 κ 分布的 MiniSelf，n=3 使用标准 Self 透镜（0.5/10/100）。实际实验若使用更极端 κ 展开（如 0.005/10/5000），GI 最优值可能微调。当前建议：n=3 时 GI=5 作为信号标定点，GI=4 作为保守运行点。
