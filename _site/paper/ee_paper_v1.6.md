...
Code & experiments: <https://github.com/JackeyLGene/GBE>
...
Three historical contexts are relevant but not required for following the experiments. Clark and Chalmers (1998) argued that cognition can extend into external media. Luhmann (1984) described society as operationally closed communication systems coupled through a shared medium. Aaronson (2011, 2013) emphasized computational cost as central to questions of mind and knowledge. The present work does not assume these theories. It provides a small computational instrument whose behavior can be tested directly.

**Contemporary dialogue.** The 2024-2026 context matters for positioning. In AI, agent-memory systems make persistent memory a first-class architectural problem: Generative Agents store natural-language experience streams and synthesize reflections (Park et al., 2023), while MemGPT manages memory tiers around a fixed context window (Packer et al., 2023). Agent evaluation work also stresses that cost, reproducibility, and hold-out discipline matter as much as headline capability (Kapoor et al., 2024). The present instrument asks a lower-level question: before a system retrieves or reflects in language, what structure should be preserved at all?

In automated science, the AI Scientist and science-exocortex proposals treat research itself as an externalized loop of ideation, coding, execution, writing, and review (Lu et al., 2024; Yager, 2024). GBE is not a general autonomous scientist; it is a small runnable artifact for testing memory pressure, compression, and inheritance. In cultural evolution, collective-brain and tacit-knowledge models study how structure survives beyond individuals (Muthukrishna and Henrich, 2016; Miton and DeDeo, 2022). The WTC experiment supplies a computational version of that transmission problem.

Finally, genomic and UTR language models show that biological sequences can be treated as structure-bearing streams (Chu et al., 2024; Sanabria et al., 2024). The biological experiments here are not offered as domain-leading biology models. They are cross-substrate stress tests for a blind structural detector. In this sense, the surrounding literatures are not targets to be displaced. They are converging on the same boundary from above: larger agents, richer memory, automated research loops, and data-intensive sequence models. This paper approaches the boundary from below, asking how much of that convergence remains visible in a minimal merge-prune instrument.
...
| DNA (§2.3) | Can evolutionary constraint be read without the genetic code? | Codon table, gene annotation, conservation scores | Exon/intron d=−0.97 (d = mean cross-harm_intron − mean cross-harm_exon; n=200) | Shuffle collapses to d=−0.10 | AHSG fork column: single-gene (n=1) |
...
**Results — C major (BWV 846).** Ten generations converge the Codex to 38 entries. Two stable patterns dominate: a tonic anchor at chroma dimension 0 (>45% of entries), and a tonic-subdominant complex at dimensions 0+5 (>55%). BiasField top dimensions: d0 (28M), d5 (5.3M), d11 (4.2M), where M denotes cumulative BiasField deposit weight in arbitrary units.

**Results — Eb major (BWV 852).** Ten generations converge to 48 entries. Four stable patterns emerge: a tonic triad skeleton spanning dimensions 2, 4, 6, 9; a dominant-tonic complex; a pure dominant anchor at dimension 9; a pure mediant anchor at dimension 4. BiasField top dimensions: d9 (1.3M), d4 (1.3M), d2 (1.2M) (M: cumulative deposit weight).
...
**Results.** Exon cross-harm (0.020) is 2.3× lower than intron cross-harm (0.045). d = −0.97 (where d = mean cross-harm_intron − mean cross-harm_exon). Shuffling genomic order collapses the signal to d=−0.10. Δd=+0.87.
...
**Results.** AHSG (alpha-2-HS-glycoprotein) — identified under positive selection in multiple genome-wide scans (Sabeti et al., 2007; Nielsen et al., 2005) — was tested as a single-gene calibration. Low-conservation columns (1-2/4 species agree) produce cross-harm 1.5 standard deviations higher than high-conservation columns (4/4 agree). d = +1.51 (where d = mean cross-harm_low-conservation − mean cross-harm_high-conservation). Shuffling column order collapses the signal to d=−0.20. Δd=+1.71. This is a single-gene result (n=1). The per-gene pooled approach did not generalize across a random sample of transcripts (mean d=0.13±0.22), indicating that the fork column signal depends on sufficient within-gene conservation variance. AHSG — the one gene in the sample with documented positive selection (Sabeti et al., 2007; Nielsen et al., 2005) — carried that variance.
...
The scientific implication is modest. The architecture does not simulate a mind, a society, a genome, or a musician. It performs one operation family under finite memory pressure and then asks whether the surviving centroids are informative. The fact that those centroids, when used as probes, reveal latent structure and, when externalized, accumulate across generations suggests that cognition may not need to begin with symbolic world-modeling. It may begin with compression, survival, and re-entry of structure.

Much perceptual research in music cognition has focused on dissonance — which chord progressions sound wrong, which intervals clash, which listener hears what in a Deutsch octave illusion (Deutsch, 1974). The present experiment asks a different question. Individual perceptual variation is not treated as the primary signal; it is treated as variation across readers. The cross-Self harm in WTC does not depend on which encoding one privileges — chroma or IOI, pitch or rhythm, ear or brain. What matters is whether the structural divergence between two readings of the same stream converges to the same centroids across generations. Deutsch's illusions measure differences between listeners. The architecture measures what survives across readings. That is the difference between studying perception and studying externalized structural cognition.

### 3.2 A Complementary Layer for Agent Memory

Base large language models are powerful recognition and generation engines. Contemporary agent architectures extend them with memory streams, reflection, retrieval, and explicit context management (Park et al., 2023; Packer et al., 2023). This field shows that persistent memory is now a central engineering problem, not something solved by increasing context length alone.

The centroid detector enters at a different layer. It does not generate language, retrieve documents, or optimize a task policy. It asks which structures survive finite-memory compression before any linguistic reflection occurs. Endogenous time, structural identity, externalized memory, and cross-generational filtering are explicit primitives rather than side effects of scale. The architecture is therefore not a competitor to LLM agents. It is a minimal testbed for the selection problem every long-memory agent eventually faces: what deserves to be written outside the current context, and how should later generations confirm, supplement, freeze, or reject it?

### 3.3 The Centroid as a Candidate Unit of Cultural Transmission

Dawkins proposed the meme as a unit of cultural transmission — replicating, mutating, selecting. The present architecture does not claim to replace that concept. It offers a lower-level operational candidate: a precipitated centroid. A precipitated frame is a centroid that survived the induction cycle. It is transmitted to the next generation through the Codex. It biases the perception of the generation that inherits it. If reinforced, its weight is maintained. If not, it decays by γ. Time does the selection.
...
Centroids precipitate into a Codex. They survive the erasure cycle. The next generation inherits them. The demonstrated operation is selection: later generations confirm, supplement, freeze, or ignore what earlier generations externalized. The centroids that survive across generations acquire operational value. Not chosen in advance. Not labeled by the experimenter. Survived. This is Create Us.
...
The architecture demonstrates that cognition need not begin with learning a world-model; it can begin with detecting structure. It is a working primitive for a non-learning cognitive architecture — a system for detecting and precipitating centroids across time. Codex formation and selection are demonstrated. Codex operation — the active loop where inherited memory stabilizes ongoing processing under noise — is the next step. We interpret centroids, accumulated across generations, as an operational form of cultural inheritance — what we provisionally call the third dimension of evolution.

The philosophical consequence is an inversion of priority. Traditional philosophy often begins with concepts — mind, world, truth, value, meaning — and asks how they relate. The present work begins with operations: merge what is similar, prune what is not reinforced, and repeat under finite memory. In the experiments above, those operations produce centroids; coupled centroids reveal structure; externalized centroids accumulate history; surviving centroids become candidates for value. Structure is therefore not treated as a ready-made property waiting to be represented. It is treated as the invariant residue of repeated cognitive operations. From so simple an economy, endless forms of structure have been, and are being, externalized.

---

## Supplementary Material

Detailed supplementary material is maintained as a separate document: `ee_supplement_v1.7.md`.
...
**Data dependencies.** Data processing scripts require: `pandas`, `numpy`, `pyarrow` (for parquet); `pybigtools` (Windows bigWig reader, RNA only; `pip install pybigtools`). These are standard scientific Python packages, not proprietary or custom toolchains. Raw data files (TE atlas parquet, bigWig tracks, GENCODE GTF, FASTA alignments) are too large for direct submission and must be obtained from their respective sources.
...
1. Aaronson, S. (2011). Why philosophers should care about computational complexity. In B. J. Copeland, C. J. Posy, & O. Shagrir (Eds.), *Computability: Turing, Gödel, Church, and Beyond*. MIT Press.
2. Aaronson, S. (2013). The ghost in the quantum Turing machine. arXiv:1306.0159.
3. Calvo, S. E., Pagliarini, D. J., & Mootha, V. K. (2009). Upstream open reading frames cause widespread reduction of protein expression and are polymorphic among humans. *PNAS*, 106(18), 7507-7512.
4. Chu, Y., Yu, D., Li, Y., Huang, K., Shen, Y., Cong, L., Zhang, J., & Wang, M. (2024). A 5′ UTR language model for decoding untranslated regions of mRNA and function predictions. *Nature Machine Intelligence*, 6, 449-460.
5. Clark, A., & Chalmers, D. (1998). The extended mind. *Analysis*, 58(1), 7-19.
6. Dawkins, R. (1976). *The Selfish Gene*. Oxford University Press.
7. Dennett, D. C. (2017). *From Bacteria to Bach and Back*. W. W. Norton.
8. Deutsch, D. (1974). An auditory illusion. *Nature*, 251, 307-309.
9. Fitch, W. T. (2010). *The Evolution of Language*. Cambridge University Press.
10. Frankish, A., et al. (2019). GENCODE reference annotation for the human and mouse genomes. *Nucleic Acids Research*, 47(D1), D766-D773.
11. Kapoor, S., Stroebl, B., Siegel, Z. S., Nadgir, N., & Narayanan, A. (2024). AI agents that matter. arXiv:2407.01502.
12. Kozak, M. (1989). The scanning model for translation: an update. *Journal of Cell Biology*, 108(2), 229-241.
13. Kozak, M. (2002). Pushing the limits of the scanning mechanism for initiation of translation. *Gene*, 299(1-2), 1-34.
14. Liu, J. (2026a). GEME: A self-referential prism for cognitive modeling. Zenodo.
15. Liu, J. (2026b). BGM: Building bridge — from Bach to bacteria and forward. Zenodo.
16. Lu, C., Lu, C., Lange, R. T., Foerster, J. N., Clune, J., & Ha, D. (2024). The AI Scientist: Towards fully automated open-ended scientific discovery. arXiv:2408.06292.
17. Luhmann, N. (1984). *Soziale Systeme*. Suhrkamp. (English: *Social Systems*, Stanford, 1995.)
18. Miton, H., & DeDeo, S. (2022). The cultural transmission of tacit knowledge. *Journal of the Royal Society Interface*, 19(195), 20220238.
19. Muthukrishna, M., & Henrich, J. (2016). Innovation in the collective brain. *Philosophical Transactions of the Royal Society B*, 371(1690), 20150192.
20. Nielsen, R., et al. (2005). A scan for positively selected genes in the genomes of humans and chimpanzees. *PLoS Biology*, 3(6), e170.
21. Packer, C., Fang, V., Patil, S. G., Lin, K., Wooders, S., & Gonzalez, J. E. (2023). MemGPT: Towards LLMs as operating systems. arXiv:2310.08560.
22. Park, J. E., Yi, H., Kim, Y., Chang, H., & Kim, V. N. (2016). Regulation of poly(A) tail and translation during the somatic cell cycle. *Molecular Cell*, 62(3), 462-471.
23. Park, J. S., O'Brien, J. C., Cai, C. J., Morris, M. R., Liang, P., & Bernstein, M. S. (2023). Generative agents: Interactive simulacra of human behavior. *Proceedings of UIST 2023*. ACM.
24. RPFdb v3. (2024). Ribosome profiling database. https://sysbio.gzzoc.com/rpfdb/.
25. Sabeti, P. C., et al. (2007). Genome-wide detection and characterization of positive selection in human populations. *Nature*, 449(7164), 913-918.
26. Sanabria, M., Hirsch, J., Joubert, P. M., & Poetsch, A. R. (2024). DNA language model GROVER learns sequence context in the human genome. *Nature Machine Intelligence*, 6, 911-923.
27. Voeten, E., Strezhnev, A., & Bailey, M. (2024). United Nations General Assembly Ideal Point Estimates. Harvard Dataverse, V33.
28. Yager, K. G. (2024). Towards a science exocortex. *Digital Discovery*, 3, 1933-1957.
